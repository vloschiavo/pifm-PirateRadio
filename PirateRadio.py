...
# Note: requires python 3.x (tested with python3.4)

# Modified by Vince Loschiavo - 2016-01-04
# - Updated to avconv since ffmpeg is deprecated
# - added the option to specify a configuration file on the command-line
# - fixed misc typos and logic errors
# - added follow-symlinks to the file parsing
# - 
...
def usage():
	print ('./PirateRadio.py -c <configfile>')
	return 0

def build_file_list():
	file_list = []
	for root, folders, files in os.walk(music_dir, followlinks=True):
		folders.sort()
		files.sort()
...
def play_songs(file_list):
	...
	if play_stereo == True:
		print("Stereo playback is " + on_off[1])
	else:
		print("Stereo playback is " + on_off[0])	
	if shuffle == True:
...
def read_config():
	...
	try:
		...
		print('Error reading from config file.', configfile)
	else:
